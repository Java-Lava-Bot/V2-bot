  ...
  async execute(client) {
    async function sendReminder(reminderData) {
      try {
        ...
        const embed = new EmbedBuilder().setTitle("⏰ Reminder!").setDescription(`This is your reminder: ${text}`).setColor("#0099ff").setTimestamp();

        await member.send({ embeds: [embed] });
        await ReminderData.deleteOne({ _id: reminderData._id });
    ...
    function scheduleReminder(reminderData) {
      const remTime = typeof reminderData.RemTime === "number" ? reminderData.RemTime : new Date(reminderData.RemTime).getTime();
    ...
    try {
      const reminders = await ReminderData.find();
      reminders.forEach(scheduleReminder);

      try {
        const changeStream = ReminderData.watch();
        ...
        logger.warn("ReminderData.watch() unavailable (requires replica set). New reminders will not auto-schedule via change streams.");
