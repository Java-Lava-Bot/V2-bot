...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      if (interaction.replied || interaction.deferred) interaction.followUp({ content: `There was an error while executing this command! Please report this to the support server ${supportinvite}`, flags: MessageFlags.Ephemeral });
      else interaction.reply({ content: `There was an error while executing this command! Please report this to the support server ${supportinvite}`, flags: MessageFlags.Ephemeral });
    }
  },
};
