const { SlashCommandBuilder, PermissionFlagsBits, EmbedBuilder, version: djsVersion, MessageFlags } = require("discord.js");
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("diagnose")
    .setDescription("Run health/config checks for the bot in this server/channel.")
    .addBooleanOption((opt) => opt.setName("public").setDescription("Post results in-channel (redacts sensitive values). Default: false (ephemeral)."))
    .addBooleanOption((opt) => opt.setName("roles").setDescription("Also check role hierarchy/ManageRoles. Default: true."))
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),

  async execute(interaction) {
    const isPublic = interaction.options.getBoolean("public") ?? false;
    const checkRoles = interaction.options.getBoolean("roles") ?? true;
    ...
    const requiredGuildPerms = [PermissionFlagsBits.ManageMessages, PermissionFlagsBits.ModerateMembers, PermissionFlagsBits.KickMembers, PermissionFlagsBits.BanMembers, ...(checkRoles ? [PermissionFlagsBits.ManageRoles] : [])];
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "Context",
          value: `• Guild: ${guild.name} (${guild.id})\n` + `• Channel: ${channel?.name ?? "—"} (${channel?.id ?? "—"})\n` + `• Bot user: ${interaction.client.user.tag} (${interaction.client.user.id})`,
          inline: false,
        },
      ...
      .setFooter({
        text: isPublic ? "Public output: sensitive values redacted" : "Private output (ephemeral)",
      });
