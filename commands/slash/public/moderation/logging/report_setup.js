...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addChannelOption((option) => option.setName("channel").setDescription("The channel where reports will be logged").setRequired(true).addChannelTypes(ChannelType.GuildText))
    .addRoleOption((option) => option.setName("role").setDescription("The role to mention when a report is made").setRequired(false))
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      const setupEmbed = new EmbedBuilder()
        ...
        .setColor("#00FF00")
        .addFields({ name: "Log Channel", value: `<#${logChannel.id}>`, inline: true }, { name: "Mention Role", value: mentionRole ? `<@&${mentionRole.id}>` : "None", inline: true }, { name: "Setup By", value: `${interaction.user.tag} (${interaction.user.id})`, inline: false }, { name: "Record ID", value: `${reportSetup?._id ?? "unknown"}`, inline: true }, { name: "Guild ID", value: `${interaction.guild.id}`, inline: true })
        .setTimestamp();
