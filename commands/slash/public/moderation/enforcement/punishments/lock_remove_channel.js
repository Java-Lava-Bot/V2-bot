...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addChannelOption((option) => option.setName("channel").setDescription("Select a channel to remove lock from").addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement, ChannelType.GuildForum).setRequired(true))
  ...
  async execute(interaction, client) {
    try {
      ...
      const logEmbed = new EmbedBuilder().setTitle("Channel Unlocked").setDescription(`${success_emote} Channel ${channel} has been unlocked by ${interaction.user}.`).setColor("Green").setTimestamp();
