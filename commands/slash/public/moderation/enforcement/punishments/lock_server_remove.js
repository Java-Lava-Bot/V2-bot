...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((option) => option.setName("include_threads").setDescription("Also restore thread posting and unlock existing active threads").setRequired(false))
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const logEmbed = new EmbedBuilder().setTitle("Server Unlocked").setDescription(`${success_emote} ${interaction.user} removed the server lock.\n\nAffected: ${affected}\nFailed: ${errors.length}`).setColor("Green").setTimestamp();
