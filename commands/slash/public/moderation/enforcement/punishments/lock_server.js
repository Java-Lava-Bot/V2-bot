...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((option) => option.setName("include_threads").setDescription("Also restrict thread posting and lock existing active threads").setRequired(false))
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const logEmbed = new EmbedBuilder().setTitle("Server Locked").setDescription(`${warning_emote} ${interaction.user} locked the server.\n\nAffected: ${affected}\nFailed: ${errors.length}`).setColor("Red").setTimestamp();
