...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addChannelOption((option) => option.setName("channel").setDescription("Select a channel to lock").addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement, ChannelType.GuildForum).setRequired(true))
  ...
  async execute(interaction, client) {
    try {
      ...
      const logEmbed = new EmbedBuilder().setTitle("Channel Locked").setDescription(`${warning_emote} Channel ${channel} has been locked by ${interaction.user}.`).setColor("Red").setTimestamp();
      ...
      try {
        if (interaction.deferred || interaction.replied) {
          ...
          await interaction.reply({ content: `${error_emote} An error occurred while executing the command. If you need assistance, please join our support server ${supportinvite}`, flags: MessageFlags.Ephemeral });
        }
      } catch {}
