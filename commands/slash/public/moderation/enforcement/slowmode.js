...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) => option.setName("duration").setDescription("Slowmode duration in seconds (0 to disable, max 21600)").setRequired(true))
    .addChannelOption((option) => option.setName("channel").setDescription("Select a channel to set slowmode for (defaults to current channel)").setRequired(false))
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      await channel.setRateLimitPerUser(duration, `Slowmode set by ${interaction.user.username} via Beta Slowmode command`);

      await interaction.editReply({ content: `${success_emote} Slowmode for channel ${channel} has been set to ${duration} seconds.` });
      const logEmbed = new EmbedBuilder().setTitle("Slowmode Updated").setDescription(`${warning_emote} Slowmode for channel ${channel} has been set to ${duration} seconds by ${interaction.user}.`).setColor("Orange").setTimestamp();
      await channel.send({ embeds: [logEmbed] });
    } catch (error) {
      logger.error(`[Beta Slowmode] Error executing command for ${interaction.user.username}: ${error?.message ?? error}`, error, client);
