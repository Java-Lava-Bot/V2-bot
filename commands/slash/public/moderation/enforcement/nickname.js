...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      await member.setNickname(newNickname, `Nickname changed by ${interaction.user.tag} using Java Lava Bot`);
      ...
      await interaction.reply({ content: `${error_emote} An error occurred while trying to change the nickname. Please ensure I have the proper permissions and the target user's highest role is below mine. If the issue persists, join our support server: ${supportinvite}`, flags: MessageFlags.Ephemeral });
    }
  },
};
