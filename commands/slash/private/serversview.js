...
module.exports = {
  ...
  data: new SlashCommandBuilder().setName("dev-guild-list").setDescription("Dev Command - List all servers the bot is in."),
    ...
    try {
      ...
      const generateEmbed = (page) => {
        ...
        return new EmbedBuilder()
          ...
          .setDescription(pageGuilds.map((guild) => `**${guild.name}**\n` + `• ID: \`${guild.id}\`\n` + `• Members: \`${guild.memberCount}\`\n` + `• Owner: <@${guild.ownerId}>\n`).join("\n") || "No guilds found")
          .setFooter({
      ...
      await interaction.reply({ content: `${error_emote} An error occurred while executing this command, check the logs for more details.`, flags: MessageFlags.Ephemeral });
    }
  },
};
